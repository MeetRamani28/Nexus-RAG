...
export const Sidebar: React.FC<Props> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0f1c] border-r border-slate-800/60">
      {/* Top Action: New Chat Button */}
      <div className="p-3 border-b border-slate-800/40">
        <button
          onClick={onNewChat}
          className="w-full flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-violet-500 text-white rounded-xl text-sm font-semibold transition-all duration-300 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 hover:scale-[1.01] active:scale-[0.99]"
        >
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-2 pb-2">
        {isLoading ? (
            ...
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="flex items-center gap-3 w-full animate-pulse">
                ...
                <div className="flex flex-col gap-1.5 flex-1">
                  ...
                  <div className="h-2.5 bg-slate-800/40 rounded-md w-1/2" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-8 px-4 animate-in fade-in">
            ...
            <p className="text-[11px] text-slate-700 mt-1">Click New Chat to begin</p>
          </div>
        ) : filtered.length === 0 ? (
          <p className="text-center text-xs text-slate-600 py-6">No results for &ldquo;{search}&rdquo;</p>
        ) : (
            ...
            <div key={label} className="mb-3">
              ...
              {convs.map(conv => (
                <div
                  ...
                  className={`group relative flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150 mb-0.5 ${
                    activeId === conv.id
                ...
                >
                  <MessageSquare className={`w-3.5 h-3.5 shrink-0 ${activeId === conv.id ? "text-blue-400" : "text-slate-600"}`} />
                  ...
                  {editId === conv.id ? (
                    <div className="flex-1 flex items-center gap-1" onClick={e => e.stopPropagation()}>
                      <input
                        ...
                        className="flex-1 bg-slate-700/80 text-slate-100 text-xs rounded-lg px-2 py-1 border border-slate-600 focus:outline-none focus:border-blue-500 min-w-0"
                      />
                      <button onClick={e => commitEdit(conv.id, e)} className="text-emerald-400 hover:text-emerald-300 p-0.5">
                        <Check className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={e => { e.stopPropagation(); setEditId(null); }} className="text-slate-500 hover:text-slate-300 p-0.5">
                    ...
                    <>
                      ...
                      <div className="hidden group-hover:flex items-center gap-1 shrink-0 ml-2">
                        ...
                        <button
                          ...
                          className="p-1 text-slate-600 hover:text-rose-400 rounded-md hover:bg-rose-500/10 transition-all"
                        >
      ...
      <div className="border-t border-slate-800/40 p-3">
        <button
          onClick={onOpenDocs}
          className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-800/30 hover:bg-slate-800/60 border border-slate-700/30 hover:border-slate-700/60 transition-all group"
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              <p className="text-xs font-medium text-slate-300">Documents</p>
              <p className="text-[10px] text-slate-500">{docCount} file{docCount !== 1 ? "s" : ""} indexed</p>
            </div>
          </div>
