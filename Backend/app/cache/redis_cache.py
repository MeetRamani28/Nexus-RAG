...
class RedisSemanticCache:
    """
    Semantic Cache layer backed by Redis.
    ...
    def __init__(self, redis_url: str = None, similarity_threshold: float = None):
        ...
        # Use shared singleton — model already loaded by QdrantVectorStore
    ...
    def _connect(self):
        ...
        if redis is None:
            print("[Redis Cache Warning]: 'redis' package is not installed. Caching disabled.")
            return
        ...
        try:
            ...
            print(f"[Redis Cache Warning]: Could not connect to Redis ({e}). Operating in pass-through mode.")
            self.client = None

    def get_cached_response(self, query: str) -> Optional[Tuple[str, List[Dict[str, Any]], float]]:
        """
        Checks Redis for semantic cache hit.
        ...
        try:
            ...
            if highest_similarity >= self.similarity_threshold and best_cached_data:
                print(f"[Redis Cache HIT]: Similarity {highest_similarity:.4f} >= {self.similarity_threshold} for query: '{query}'")
                return (
            ...
            print(f"[Redis Cache MISS]: Max similarity {highest_similarity:.4f} < {self.similarity_threshold}")
            return None
    ...
    def set_cached_response(
        ...
        """
        Caches a query, its embedding vector, answer generation, and citations in Redis.
        """
        if not self.client:
            return
        ...
        try:
            ...
            print(f"[Redis Cache SET]: Cached response for query: '{query}' (TTL: {ttl_seconds}s)")
        except Exception as e:
